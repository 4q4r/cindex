        ...
        for attempt in range(1, self.MAX_ATTEMPTS + 1):
            try:
                ...
                body, status = self._resolve_cloudflare_challenge(
                    scraper, url, params, body, status,
                )
                if status >= 400:
                    raise ConnectorFetchError(
...
class AsyncApiConnector(BaseConnector):
    ...
    async def _fetch_async(self, query: str, limit: int) -> list[RawArticle]:
        ...
        try:
            ...
            raise ConnectorFetchError(
                f"{self.profile.source_key}: HTTP {status} for {url}: {exc}",
            ) from exc
        except aiohttp.ClientError as exc:
            raise ConnectorFetchError(
                f"{self.profile.source_key}: request failed: {exc}",
            ) from exc
        if not isinstance(payload, dict):
            raise ConnectorFetchError(
                f"{self.profile.source_key}: invalid JSON payload type",
    ...
    def enrich_raw(self, raw: RawArticle) -> RawArticle:
        """API connectors typically return complete metadata; no enrichment needed."""
        return raw
