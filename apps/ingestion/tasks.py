...
@shared_task(
    autoretry_for=(Exception,), retry_backoff=True, retry_jitter=True, max_retries=5,
)
def ingest_search_query(
    query: str, source_keys: list[str] | None = None,
) -> dict[str, int]:
    """Ingest search query helper."""
    run = IngestionRun.objects.create(
        query=query, source_key=",".join(source_keys or ["all"]),
    )
    try:
...
def doi_enrichment_backfill(batch_size: int = 200) -> dict[str, int]:
    ...
    candidates = DoiEnrichmentService._select_doi_candidates()[:batch_size]
    if not candidates:
