  ...
  return (
    <div
      className={`flex items-center gap-1.5 px-2 py-0.5 rounded text-[11px] font-medium ${
        value ? 'bg-success-muted/40 text-success' : 'bg-bg-elevated text-text-tertiary'
      }`}
    >
...
export function ResultCard({ result, query, viewMode, onCopy }: ResultCardProps) {
  const [expanded, setExpanded] = useState(false);
  const [showCitation, setShowCitation] = useState(false);

  const isCompact = viewMode === 'compact';
  const identifiers = Object.entries(result.identifiers || {});
  ...
  return (
    ...
    >
      ...
      <div className={`text-sm text-text-secondary ${isCompact ? 'mb-2' : 'mb-3'} leading-relaxed`}>
        {result.year ? <span className="text-text-primary font-medium">{result.year}</span> : <span>Год не указан</span>}
      ...
      {identifiers.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-3">
          {identifiers.map(([label, value]) => (
            <span key={label} className="text-[11px] text-text-tertiary bg-bg-elevated px-2 py-0.5 rounded">
              {label}: <span className="text-text-secondary font-mono">{value}</span>
            </span>
          ))}
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-1.5 mb-4">
        <CriterionBadge label="DOI и карточка" value={result.eligibilityEvidence.doiAndJournalCard} icon={Hash} />
        <CriterionBadge label="Peer-reviewed" value={result.eligibilityEvidence.peerReviewed} icon={Shield} />
        <CriterionBadge label="Индексация" value={result.eligibilityEvidence.indexed} icon={Database} />
        <CriterionBadge label="Не preprint" value={result.eligibilityEvidence.notPreprint} icon={FileText} />
      </div>

      <div className={`text-sm text-text-secondary leading-relaxed ${isCompact ? 'line-clamp-5' : ''} mb-4`}>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <button
          onClick={() => onCopy(result.citationFull, 'citation')}
      ...
      <div className="flex items-center gap-4 mt-3 text-[11px] text-text-tertiary">
        <div className="flex items-center gap-1">
          ...
          <span>Проверка критериев: {result.eligibilityConfidence.overall}%</span>
        </div>
      </div>
      ...
      {expanded && (
        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-text-secondary">
          ...
          <div>DOI/карточка: {result.eligibilityConfidence.doiAndJournalCard}%</div>
